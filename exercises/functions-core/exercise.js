...
function es5(person, greeting) {
  // greeting = greeting || 'hi';

  greeting = greeting == null ? 'hi' : greeting;
  return greeting + " " + person;
}
...
export function englishList(words, { oxford = false } = {}) {
  const [first,second] = words;
  const [last, ...prefix] = words.reverse();
  const suffix = oxford ? "," : "";
  ...
  switch(words.length) {
    ...
    default:
      return `${prefix.reverse().join(", ")}${suffix} and ${last}`;
  }
}
